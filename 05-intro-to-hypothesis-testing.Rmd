...
So, we can collect data, but what about drawing inferences and carrying out significance tests? A **significance test** can tell us whether an observed effect (e.g., the difference between two means, or medians; the correlation between two variables) could occur by chance in selecting a random sample from each of, say, two populations.
...
<li> List the aims, write out the appropriate null and alternative hypothesis using statistical notation for, and write `R` code to carry out a randomization test </li>
<li> Correctly interpret and communicate a p-value in terms of a randomisation test</li>
...
The basic approach to a randomisation test is straightforward:
...
But because the data are skewed and we've likely got non-constant variances we may be better off adopting a randomisation test, rather than a parametric t-test (see the next chapter). Our approach is as follows.
...
## What is a `r emo::ji('scream')` p-value? `r emo::ji('scream')` Really?
...
**Q:** Why do so many colleges and grad schools teach *p-val*=0.05? **A:** Because that's still what the scientific community and journal editors use. `r emo::ji('scream')` **BUT IT SHOULDN'T BE** `r emo::ji('scream')`

**Q:** Why do so many people still use *p-val*=0.05? **A:** Because that's what they were rote taught in college or grad school. `r emo::ji('scream')`**BUT THEY SHOULDN'T BE**`r emo::ji('scream')`
